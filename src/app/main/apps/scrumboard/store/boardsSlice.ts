 ...
 * Get Boards
 */
export const getBoards = createAppAsyncThunk<BoardsType>(
...
export const newBoard = createAppAsyncThunk<BoardType, PartialDeep<BoardType>>(
    ...
    async (board) => {
        const response = await axios.post('/api/scrumboard/boards', board);
        const data = (await response.data) as BoardType;

        history.push({
            pathname: `/apps/scrumboard/boards/${data.board_id}`,
        });
