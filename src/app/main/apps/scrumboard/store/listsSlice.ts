 ...
 * Get Board Lists
 */
export const getLists = createAppAsyncThunk<ListsType, string>(
...
export const newList = createAppAsyncThunk<ListType, Partial<ListType>>(
    ...
    async (list, { getState }) => {
        ...
        const response = await axios.post(`/api/scrumboard/boards/${board.board_id}/lists`, list);
...
export const updateList = createAppAsyncThunk<ListType, { id: string; newData: Partial<ListType> }>(
    ...
    async ({ id, newData }, { getState }) => {
        ...
        const response = await axios.put(
            `/api/scrumboard/boards/${board.board_id}/lists/${id}`,
            newData,
        );
...
export const removeList = createAppAsyncThunk<string, string>(
    ...
    async (id, { getState }) => {
        ...
        const response = await axios.delete(`/api/scrumboard/boards/${board.board_id}/lists/${id}`);
...
export const selectListById = (list_id: ListType['list_id']) => (state: AppRootStateType) =>
    selectById(state, list_id);
