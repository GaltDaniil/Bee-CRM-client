...
class JwtService extends FuseUtils.EventEmitter {
     ...
     */
    createUser = (data: {
        user_name: UserType['data']['user_name'];
        user_password: string;
        user_email: UserType['data']['user_email'];
    }) =>
        new Promise((resolve, reject) => {
            ...
            axios.post('api/auth/sign-up', dataForServer).then(
                (
                    ...
                    if (response.data.user) {
                        console.log('прошло', response.data);
     ...
     */
    signInWithEmailAndPassword = (user_email: string, user_password: string) =>
        new Promise((resolve, reject) => {
            axios
...
function isAuthTokenValid(access_token: string) {
    ...
    console.log('Получен токен из локала', decoded);
